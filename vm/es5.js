...
module.exports = {
  ...
  IfStatement(path) {
    const ifScope = path.scope.createChild(Types.If);
  ...
  BlockStatement(path) {
    ...
    let blockScope = !scope.isolated ?
      scope :
      scope.createChild(Types.Block);

    if (scope.isolated) {
      blockScope = scope.createChild(Types.Block);
  ...
  LabeledStatement(path) {
    ...
    return path.evaluate(
      path.createChild(path.node.body, path.scope, {
  ...
  VariableDeclaration(path) {
    ...
    for (const declaration of node.declarations) {
      ...
      for (const varName in varKeyValueMap) {
         ...
         * }
         */
        if (scope.invasive && kind === Types.Var) {
          const targetScope = (function get(s) {
            if (s.parent) {
              if (s.parent.invasive) {
                return get(s.parent);
  ...
  VariableDeclarator(path) {
    ...
    if (isObjectPattern(node.id)) {
      const newScope = scope.createChild(Types.Object);
  ...
  FunctionDeclaration(path) {
    ...
    if (node.async) {
        ...
        return __awaiter(this, void 0, void 0, () => {
          ...
          function handler(_a) {
            ...
            const r = path.evaluate(
              path.createChild(block, path.scope, {
                next
              })
            );
      ...
      func = es5.FunctionExpression(path.createChild(node));
    }
  ...
  ForStatement(path) {
    ...
    const forScope = scope.createChild(Types.For);
    ...
    for (;;) {
      ...
      const loopScope = forScope.fork(Types.ForChild);
      ...
      const signal = path.evaluate(
        path.createChild(node.body, loopScope, {
          labelName: undefined
        })
      );
  ...
  ForInStatement(path) {
    ...
    for (const value in right) {
      if (Object.hasOwnProperty.call(right, value)) {
        const forInScope = scope.createChild(Types.ForIn);
  ...
  DoWhileStatement(path) {
    ...
    do {
      const doWhileScope = scope.createChild(Types.DoWhile);
  ...
  WhileStatement(path) {
    ...
    while (path.evaluate(path.createChild(node.test))) {
      const whileScope = scope.createChild(Types.While);
  ...
  TryStatement(path) {
    ...
    try {
      ...
      if (node.finalizer) {
        const finallyScope = scope.createChild(Types.Finally);
  ...
  SwitchStatement(path) {
    ...
    const switchScope = scope.createChild(Types.Switch);
  ...
  ThisExpression(path) {
    ...
    if (scope.type === Types.Constructor) {
      if (!scope.hasOwnBinding(Types.THIS)) {
        throw overriteStack(ErrNoSuper(), path.stack, path.node);
      }
    }
    const thisVar = scope.hasBinding(Types.THIS);
  ...
  ArrayExpression(path) {
    ...
    for (const item of node.elements) {
      if (item === null) {
        ...
        newArray = [].concat(newArray, _toConsumableArray(arr));
      } else {
  ...
  ObjectExpression(path) {
    ...
    for (const property of node.properties) {
      ...
      path.evaluate(path.createChild(property, newScope, {
        object
      }));
    }
  ...
  ObjectMethod(path) {
    ...
    const method = function () {
      ...
      const newScope = scope.createChild(Types.Function);
      newScope.const(Types.THIS, this);
      // define arguments
    ...
    const objectKindMap = {
      get() {
        Object.defineProperty(path.ctx.object, methodName, {
          get: method
        });
  ...
  FunctionExpression(path) {
    ...
    const func = function (...args) {
      ...
      const funcScope = scope.createChild(Types.Function);
      for (let i = 0; i < node.params.length; i++) {
        ...
        if (isIdentifier(param)) {
          ...
          path.evaluate(path.createChild(param, funcScope, {
            value: args[i]
          }));
        } else if (isRestElement(param)) {
          // @es2015 rest parameters
          path.evaluate(
            path.createChild(param, funcScope, {
              value: args.slice(i)
            })
          );
        }
      ...
      funcScope.const(Types.THIS, this);
      // support new.target
      funcScope.const(Types.NEW, {
        target: this && this.__proto__ && this.__proto__.constructor ?
          this.__proto__.constructor :
          undefined
      });
      funcScope.const(Types.ARGUMENTS, arguments);
    ...
    return {
      ...
      typeof: () => {
        if (isIdentifier(node.argument)) {
          ...
          return $var ? typeof $var.value : Types.UNDEFINED;
        } else {
      ...
      delete: () => {
        if (isMemberExpression(node.argument)) {
          ...
          const $this = scope.hasBinding(Types.THIS);
          if ($this) {
  ...
  CallExpression(path) {
    ...
    const functionName = isMemberExpression(node.callee) ?
      (() => {
    ...
    const isValidFunction = isFunction(func);
  ...
  MemberExpression(path) {
    ...
    const target = isPrototype ? new Object(obj) : obj[propertyName];
...
exports.BinaryExpressionOperatorEvaluateMap = BinaryExpressionOperatorEvaluateMap
exports.AssignmentExpressionEvaluateMap = AssignmentExpressionEvaluateMap